...
def _risk_gauge(confidence: float, risk_category: str) -> None:
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            title={
                "text": f"Confidence Â· {risk_category}",
                "font": {
                    "size": 11,
                    "color": "#8b949e",
                    "family": t["font_family"],
...
def _vehicle_radar(input_payload: dict, automobile: pd.DataFrame) -> None:
    ...
    for label, (column, value) in features.items():
        if column in automobile.columns:
            numeric_column = pd.to_numeric(automobile[column], errors="coerce")
            maxes[label] = float(numeric_column.max()) or 1
            means[label] = float(numeric_column.mean())
        else:
            maxes[label] = value * 2 or 1
            means[label] = value
    ...
    def normalize(values):
        return [
            value / maxes[category] if maxes[category] > 0 else 0
            for value, category in zip(values, categories)
        ]

    input_normalized = normalize([features[category][1] for category in categories])
    average_normalized = normalize([means[category] for category in categories])
    ...
    fig.add_trace(
        go.Scatterpolar(
            r=average_normalized + [average_normalized[0]],
            theta=closed_categories,
            fill="toself",
...
def _vehicle_3d_scatter(
    automobile: pd.DataFrame,
    input_payload: dict,
    risk_category: str,
) -> None:
    required_columns = ["horsepower", "curb-weight", "city-mpg"]

    if not all(column in automobile.columns for column in required_columns):
    ...
    color_values = df[color_column].fillna(df[color_column].median())
    ...
    fig.update_layout(
        scene=dict(
            xaxis=axis_style("Horsepower"),
            yaxis=axis_style("Curb Weight (lbs)"),
            zaxis=axis_style("City MPG"),
            bgcolor="#0d1117",
        ...
        title=dict(
            text="3D Vehicle Space â€” Horsepower Ã— Curb Weight Ã— City MPG",
...
def render_vehicle_risk_page() -> None:
    page_header(
        "Vehicle Risk Scoring Â· UCI Automobile Model",
    ...
    section_header("âš™ï¸", "Vehicle Configuration")
    ...
    if predict_button:
        st.session_state["total_predictions_made"] = (
            st.session_state.get("total_predictions_made", 0) + 1
        )

        with st.spinner("Computing risk score..."):
            st.session_state["vehicle_prediction"] = predict_vehicle_risk(
                input_payload
            )
            st.session_state["vehicle_neighbors"] = similar_vehicle_profiles(
                input_payload
            )
    ...
    section_header("â—»", "3D Vehicle Feature Space")
    _vehicle_3d_scatter(automobile, input_payload, risk_category)

    if prediction:
        result_column, radar_column = st.columns([1, 1])

        with result_column:
            section_header("â—†", "Risk Score Result")
            ...
            for feature in prediction.get("top_contributing_features", [])[:6]:
                importance = float(feature.get("importance", 0))
                bar_width = min(100, int(importance * 500))

                st.markdown(
                    f"""
                    <div style="margin:8px 0">
                        <div style="display:flex;justify-content:space-between;margin-bottom:3px">
                            <span style="font-size:0.8125rem;color:#8b949e">
                                {feature.get("feature", "")}
                            </span>
        ...
        with radar_column:
            section_header("â—ˆ", "Vehicle Profile Radar")
            _vehicle_radar(input_payload, automobile)

        section_header("â—»", "Similar Vehicles")

        neighbors = st.session_state.get(
            "vehicle_neighbors",
            pd.DataFrame(),
        )
        ...
        if not neighbors.empty:
            ...
            st.info("No similar vehicle profiles available yet.")
